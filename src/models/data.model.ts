...
export class Data {
      ...
      this.client.get(`http://localhost:3000/users/${id}`, options)
        .subscribe((response: ServerUserObject) => {
          const clientUserObject = new User(response.email,
            ...
            response.phone,
            response.location);
          this.addFieldsNotRequired(response, clientUserObject);
  ...
  getRentals(): Promise<Array<Rental>> {
    return new Promise((resolve) => {
      this.client.get(`http://localhost:3000/rentals`)
        .subscribe(async (response: Array<ServerRentalObject>) => {
          ...
          for (let i = 0; i < response.length; i++) {
            ...
            rental.id = curr.id;
            const hostUser: User = await this.getUserByIdUnproteted(curr.hostID);
            rental.hostName = `${hostUser.getFirstName()} ${hostUser.getLastName()}`;
            array.push(rental);
          }
          resolve(array);
        });
